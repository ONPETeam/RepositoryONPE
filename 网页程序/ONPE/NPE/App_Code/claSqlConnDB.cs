...
{
    // ＝＝＝数据库连接串设置＝＝＝
    public static readonly string gStrConnDefault = System.Configuration.ConfigurationManager.AppSettings["DataBaseConnect"];
    //public static readonly string gStrConnDefault = @"Password=Gjhf-1478963250;Persist Security Info=True;User ID=sa;Initial Catalog=ONPE;Data Source=121.40.203.230"; // 系统默认数据库连接串
    //public static readonly string gStrConnDefault = @"Password=1478963250;Persist Security Info=True;User ID=sa;Initial Catalog=WGNHNPE;Data Source=192.168.1.250"; // 系统默认数据库连接串
    //public static readonly string gStrConnDefault = @"Password=Gjhf1478963250;Persist Security Info=True;User ID=sa;Initial Catalog=ONPE;Data Source=172.16.60.202"; 
    //有用、常用的方法和结构体
    ...
    /// <returns></returns>
    public static Object GetDetail(string conn_Default, string vStrSQL, string class_Name, SqlParameter[] prams)
    {
        // ＝＝＝获得数据库源，返回IList为数据源＝＝＝
        IList Ilst = new ArrayList();
        ...
        using (DataSet ds = ExecuteDataset(conn_Default, CommandType.Text, vStrSQL, prams))
        {
            ...
            for (int j = 0; j < dt.Rows.Count; j++)
            {
                System.Type myType = Type.GetType(class_Name);
                Object o_Instance = System.Activator.CreateInstance(myType); // 实例化类
                // 获得类的所有属性数组
                PropertyInfo[] myPropertyInfo1 = myType.GetProperties(BindingFlags.Public | BindingFlags.Instance);
                // 循环属性数组，并给数组属性赋值
